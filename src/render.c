...
/*   Updated: 2024/03/18 17:25:51 by thibault         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	img_pix_put(t_canevas *cnv, int x, int y, int color)
{
	...
	addr = cnv->img->addr;
	len = cnv->img->line_len;
	pixel = 0;
	if (x >= 0 && x < cnv->win_width && y >= 0 && y < cnv->win_height)
	{
		pixel = addr + (y * len + x * (cnv->img->bpp / 8));
...
t_param	hit_get_param(t_hit *hit, t_ray ray)
{
	...
	// DEBUG
	if (0 && hit_param.object->sphere.radius == 1)
	{
